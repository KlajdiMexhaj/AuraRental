...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    { name: t("nav.discover"), path: "/", onClick: handleDiscoverClick },
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${scrolled ? "py-4" : "py-8"}`}>
      <div className="max-w-7xl mx-auto px-6 transition-all duration-500">
        ...
        <div
          className={`flex justify-between items-center rounded-3xl transition-all duration-500 px-8 ${
            scrolled
              ? "bg-[#0b1c1c]/80 backdrop-blur-2xl border border-white/5 shadow-2xl py-3"
              : "bg-transparent py-0"
          }`}
        >
          ...
          >
            ...
            <span className="text-xl font-bold tracking-tight text-white flex flex-col leading-none">
              AURA
              <span className="text-[#8ecd24] text-[10px] tracking-[0.4em] font-black uppercase">
                {t("nav.brand")}
              </span>
            </span>
          </Link>
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed inset-0 bg-[#011111] z-[90] transition-all duration-500 ease-expo ${
