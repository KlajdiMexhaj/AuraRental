...
const CarList: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="mb-12">
        <h1 className="text-4xl md:text-5xl font-black text-white mb-4">{t('collection.title')}</h1>
        <p className="text-gray-500 max-w-2xl text-lg">
          {searchParams.get('pickup') 
            ? t('collection.availableDates')
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-40 gap-4">
          ...
          <p className="text-[#8ecd24] font-medium animate-pulse">{t("collection.scanning")}</p>
        </div>
      ) : error ? (
        <div className="text-center py-20 bg-[#0b1c1c] rounded-3xl border border-red-500/20">
          <p className="text-red-400 mb-4">{error}</p>
          <button onClick={() => window.location.reload()} className="bg-[#8ecd24] text-[#011111] px-6 py-2 rounded-full font-bold">{t("collection.tryAgain")}</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {cars.length === 0 && (
            <div className="col-span-full text-center py-32 bg-[#0b1c1c] rounded-[40px] border border-white/5">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">{t("collection.noCars")}</h3>
              <p className="text-gray-500">{t("collection.adjustDates")}</p>
              <button 
